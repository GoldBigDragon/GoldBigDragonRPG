...
public class DungeonChat
{
	...
	private void DungeonMainChatting(AsyncPlayerChatEvent event)
	{
		...
		if(u.getString(player,(byte)0).equals("ND"))
		{
			...
			{
				dungeonListYaml.set(Message, servertick.ServerTickMain.nowUTC);
				...
				dungeonListYaml.set("Type.Name", main.MainServerOption.dungeonTheme.get(0));
		...
		else if(u.getString(player,(byte)0).equals("DS"))
		{
			if(isIntMinMax(event.getMessage(), player, 5, 50))
			{
				...
				dungeonListYaml.saveConfig();
				SoundEffect.playSound(player, Sound.ENTITY_ITEM_PICKUP, 1.0F, 1.0F);
				new dungeon.DungeonGui().DungeonSetUpGUI(player, u.getString(player, (byte)1));
				u.clearAll(player);
			}
		}
	...
	private void EnterCardChatting(AsyncPlayerChatEvent event)
	{
		...
		if(u.getString(player,(byte)0).equals("NEC"))
		{
			...
			u.clearAll(player);
			new dungeon.DungeonGui().DungeonListMainGUI(player, 0, 358);
			return;
		}
		...
		else if(u.getString(player,(byte)0).equals("ECDATA"))
		{
			if(isIntMinMax(event.getMessage(), player, 0, 20))
			{
				...
				enterCardListYaml.saveConfig();
				SoundEffect.playSound(player, Sound.ENTITY_ITEM_PICKUP, 1.0F, 1.0F);
				new dungeon.DungeonGui().EnterCardSetUpGUI(player, u.getString(player, (byte)1));
				u.clearAll(player);
			}
		}
	...
	private void AltarChatting(AsyncPlayerChatEvent event)
	{
		...
		if(u.getString(player,(byte)0).equals("NA_Q"))
		{
			...
			{
				SoundEffect.playSound(player, Sound.ENTITY_ITEM_PICKUP, 1.0F, 1.2F);
				u.clearAll(player);
				new dungeon.DungeonGui().AltarShapeListGUI(player);
			}
				...
				||Message.equals("X"))
			{
				...
				u.clearAll(player);
				new dungeon.DungeonGui().DungeonListMainGUI(player, 0, 120);
			}
			return;
		}
		else if(u.getString(player,(byte)0).equals("EAN"))
		{
			...
			SoundEffect.playSound(player, Sound.ENTITY_ITEM_PICKUP, 1.0F, 1.0F);
			new dungeon.DungeonGui().AltarSettingGUI(player, AltarName);
			u.clearAll(player);
		}
	}
