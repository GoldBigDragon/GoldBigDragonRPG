...
public class UserObject
{
	...
	public boolean addStat_MoneyAndEXP(long Money, long EXP, boolean isAlert)
	{
			...
			for(;;)
			{
				...
				else
				{
					...
					if(MainServerOption.maxLevel <= Stat_Level)
						new effect.SendPacket().sendActionBar(Bukkit.getPlayer(PlayerName), "§c§l[최대 레벨에 도달하여 더이상 레벨업 하실 수가 없습니다!]", false);
				}
			}
			if(isLevelUp)
			{
		...
		{
			if(Money!=0&&EXP!=0)
				new effect.SendPacket().sendActionBar(player, "§b§l[경험치] "+ EXP + " §e§l[§f"+MainServerOption.money+"§e§l] "+ Money, false);
			else if(Money!=0)
				new effect.SendPacket().sendActionBar(player, "§e§l[§f"+MainServerOption.money+"§e§l] "+ Money, false);
			else
				new effect.SendPacket().sendActionBar(player, "§b§l[경험치] "+ EXP, false);
		}
