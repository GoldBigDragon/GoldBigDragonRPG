...
public class ServerTickObject
{
	...
	public ServerTickObject(long Tick, String Type)
	{
		this.Tick = Tick;
		this.Type = Type;
		for(int count = 0; count < this.Stringa.length;count++)
			this.Stringa[count] = null;
		for(int count = 0; count < this.Int.length;count++)
			this.Int[count] = 0;
		for(int count = 0; count < this.Boolean.length;count++)
			this.Boolean[count] = false;
	}
	
	public void copyThisScheduleObject(long WillAddTick)
	{
		servertick.ServerTickMain.Schedule.put(WillAddTick, this);
	}
