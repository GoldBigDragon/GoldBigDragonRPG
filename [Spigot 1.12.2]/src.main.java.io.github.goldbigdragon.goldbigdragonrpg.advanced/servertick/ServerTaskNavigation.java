...
public class ServerTaskNavigation
{
	...
	public void Routing(ServerTickObject STSO)
	{
			...
			if(Bukkit.getServer().getPlayer(STSO.getString((byte)2)).isOnline())
			{
				...
				if(DestinationLoc.getWorld()!=SourceLoc.getWorld())
				{
					...
					if(DestinationLoc.getWorld().getName().equals("world_nether"))
						PS.sendTitle(player, "§e", "§e[§c네더(지옥) 월드§e로 이동하세요.]", 1, 1, 1);
					else if(DestinationLoc.getWorld().getName().equals("world_the_end"))
						PS.sendTitle(player, "§e", "§e[§7엔더 월드§e로 이동하세요.]", 1, 1, 1);
					else
						PS.sendTitle(player, "§e", "§e[§f"+DestinationLoc.getWorld().getName()+"§e 월드로 이동하세요.]", 1, 1, 1);
	...
	public void FindedWay(Player player, Location DestinationLoc)
	{
		...
		PS.sendTitle(player, "§e도착하였습니다!", "§f[네비게이션을 초기화 합니다.]", 1, 1, 1);
