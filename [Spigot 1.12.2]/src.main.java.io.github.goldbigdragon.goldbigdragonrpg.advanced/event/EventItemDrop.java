...
public class EventItemDrop
{
	...
	public void MoneyDrop(Location loc, int money)
	{
		...
		if(money <= 0)
		return;
	    int mok = money/configYaml.getInt("Server.Max_Drop_Money");
	    //if(mok >= 20)
	    //	mok = 20;
	    for(int count = 0; count < mok; count++)
		    loc.getWorld().dropItemNaturally(loc, MoneyShape(configYaml.getInt("Server.Max_Drop_Money")));

	    int na = money%configYaml.getInt("Server.Max_Drop_Money");
	    if(na >= 1)
	...
	{
		main.MainServerOption SO = new main.MainServerOption();
		ItemStack Item;
		if(money<=50)
		...
		ItemMeta Item_Meta = Item.getItemMeta();
		Item_Meta.setDisplayName(main.MainServerOption.money+" 」f」f」f」l" + money);
		Item_Meta.setLore(Arrays.asList("」e"+ money +" "+main.MainServerOption.money,passmoney+""));
