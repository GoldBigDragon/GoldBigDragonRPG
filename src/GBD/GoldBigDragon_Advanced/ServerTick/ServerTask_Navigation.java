...
public class ServerTask_Navigation
{
			...
			if(Bukkit.getServer().getPlayer(STSO.getString((byte)2)).isOnline())
			{
				...
				if(DestinationLoc.getWorld()!=SourceLoc.getWorld())
				{
					...
					if(DestinationLoc.getWorld().getName().equals("world_nether"))
						PS.sendTitleSubTitle(player, "\'"+ChatColor.YELLOW+""+"\'", "\'"+ChatColor.YELLOW+"["+ChatColor.RED+"네더(지옥) 월드"+ChatColor.YELLOW+"로 이동하세요.]"+"\'", 1, 1, 1);
	...
	public void ShowWay(Player player,String Way, int y,int arrowtype)
	{
		switch(arrowtype)
		{
		default:
	...
	{
		GBD.GoldBigDragon_Advanced.Effect.PacketSender PS = new GBD.GoldBigDragon_Advanced.Effect.PacketSender();
		new GBD.GoldBigDragon_Advanced.Effect.Sound().SP(player, Sound.NOTE_PLING, 1.0F, 1.0F);
		PS.sendTitleSubTitle(player, "\'"+ChatColor.YELLOW+"도착하였습니다!"+"\'", "\'"+ChatColor.WHITE+"[네비게이션을 초기화 합니다.]"+"\'", 1, 1, 1);
