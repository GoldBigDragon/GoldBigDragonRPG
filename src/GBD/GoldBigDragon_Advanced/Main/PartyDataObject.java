...
public class PartyDataObject
{
	...
	public PartyDataObject(Long CreateTime, Player player, String PartyName)
	{
		...
		Main.PartyJoiner.put(player, CreateTime);
		Main.Party.put(CreateTime, this);
	}
	
	public PartyDataObject(Long CreateTime, String Leader,
			String Title, Boolean PartyLock, String PartyPassword, int PartyCapacity,
			String[] PartyMember)
	{
		this.Title = Title;
		this.CreateTime = CreateTime;
	...
	public void ChangeMaxCpacity(Player player, int Capacity)
	{
		if(player.getName().equals(this.Leader))
			if(Capacity >= getPartyMembers())
			{
				if(Capacity >= 2)
				{
					...
					YamlManager Config = Party_YC.getNewConfig("config.yml");
						...
						for(int count = 0; count < this.PartyCapacity; count++)
							if(this.PartyMember[count] == null)
							{
								if(player.isOnline())
								{
									...
									new GBD.GoldBigDragon_Advanced.Effect.Sound().SP(player, Sound.DOOR_OPEN, 1.1F, 1.0F);
								}
								this.PartyMember[count] = player.getName();
								Main.PartyJoiner.put(player, this.CreateTime);
	...
	{
		Main.PartyJoiner.remove(player);
		if(player.isOnline())
		{
			player.sendMessage(ChatColor.RED + "[파티] : 파티를 탈퇴하였습니다!");
			new GBD.GoldBigDragon_Advanced.Effect.Sound().SP(player, Sound.DOOR_CLOSE, 1.1F, 1.0F);
		}
