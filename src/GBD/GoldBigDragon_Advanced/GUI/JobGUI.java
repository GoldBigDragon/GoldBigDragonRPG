...
public class JobGUI extends GUIutil
{
	...
	public void MapleStory_JobSetting(Player player, String JobName)
	{
		...
		for(int count = 0; count < b.size();count++)
		{
			...
			if(count == 0)
				Stack2(ChatColor.WHITE + "" + ChatColor.BOLD + JobNick , IconID,IconData,IconAmount,Arrays.asList(ChatColor.DARK_AQUA+"등록된 스킬 수 : "+ChatColor.WHITE+JobSkillList+ ChatColor.DARK_AQUA+"개",ChatColor.GREEN+"승급 필요 레벨 : "+ChatColor.WHITE+NeedLevel+ChatColor.GREEN+" 이상",ChatColor.GREEN+"승급 필요 "+GBD.GoldBigDragon_Advanced.Main.ServerOption.STR+" : "+ChatColor.WHITE+NeedSTR+ChatColor.GREEN+" 이상",ChatColor.GREEN+"승급 필요 "+GBD.GoldBigDragon_Advanced.Main.ServerOption.DEX+" : "+ChatColor.WHITE+NeedDEX+ChatColor.GREEN+" 이상",ChatColor.GREEN+"승급 필요 "+GBD.GoldBigDragon_Advanced.Main.ServerOption.INT+" : "+ChatColor.WHITE+NeedINT+ChatColor.GREEN+" 이상",ChatColor.GREEN+"승급 필요 "+GBD.GoldBigDragon_Advanced.Main.ServerOption.WILL+" : "+ChatColor.WHITE+NeedWILL+ChatColor.GREEN+" 이상",ChatColor.GREEN+"승급 필요 "+GBD.GoldBigDragon_Advanced.Main.ServerOption.LUK+" : "+ChatColor.WHITE+NeedLUK+ChatColor.GREEN+" 이상"	,ChatColor.GREEN+"이전 승급 단계 : "+ChatColor.WHITE+PrevJob,"",ChatColor.YELLOW+"[좌 클릭시 직업 스킬 등록]",ChatColor.YELLOW+"[우 클릭시 직업 스킬 확인]",ChatColor.YELLOW+"[Shift + 좌 클릭시 승급 제한 설정]",ChatColor.YELLOW+""+ChatColor.BOLD+"[기본 클래스]"), count, inv);
			else
