	...
	public void PartyGUI_Main(Player player)
	{
		...
		else
		{
			...
			if(Main.Party.get(Main.PartyJoiner.get(player)).getLeader().equalsIgnoreCase(player.getName()) == true)
			{
				...
				if(Main.Party.get(Main.PartyJoiner.get(player)).getLock() == false) Stack2(ChatColor.BLUE +""+ ChatColor.BOLD + "파티 개방", 54,0,1,Arrays.asList(ChatColor.GRAY + "파티 가입 신청을 받습니다."), 34, inv);
	...
	public void PartyListGUI(Player player, int page)
	{
		...
		else
		{
			...
			for(int count = page*45; count < Main.Party.size();count++)
			{
				...
				Stack2(ChatColor.WHITE + "" + ChatColor.BOLD + Main.Party.get(a[count]).getTitle(), 54,0,1,Arrays.asList(ChatColor.GRAY + "파티 생성 시각 : "+ChatColor.DARK_GRAY+a[count],"",ChatColor.AQUA + "파티장 : "+ChatColor.GRAY+Main.Party.get(a[count]).getLeader(),ChatColor.AQUA + "인원 : "+ChatColor.GRAY + Main.Party.get(a[count]).getPartyMembers()+"/"+Main.Party.get(a[count]).getCapacity()), loc, inv);
				else
				Stack2(ChatColor.WHITE + "" + ChatColor.BOLD + Main.Party.get(a[count]).getTitle(), 130,0,1,Arrays.asList(ChatColor.GRAY + "파티 생성 시각 : "+ChatColor.DARK_GRAY+a[count],"",ChatColor.AQUA + "파티장 : "+ChatColor.GRAY+Main.Party.get(a[count]).getLeader(),ChatColor.AQUA + "인원 : "+ChatColor.GRAY + Main.Party.get(a[count]).getPartyMembers()+"/"+Main.Party.get(a[count]).getCapacity(),"",ChatColor.RED + "[잠김]",ChatColor.RED + "파티 가입을 하시려면",ChatColor.RED +"파티장에게 문의하세요!"), loc, inv);
				loc=loc+1;
			}
		}
