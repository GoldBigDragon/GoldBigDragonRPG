...
{
    public void NPCscriptExample()
	{
		YamlController Config_YC = GoldBigDragon_RPG.Main.Main.YC_1;
		YamlManager NPCscript=Config_YC.getNewConfig("NPC/NPCscriptExample.yml");
    ...
    public void PlayerNPCconfig(Player player, String NPCuuid)
	{
		...
		YamlController Config_YC = GoldBigDragon_RPG.Main.Main.YC_1;
    	if(Config_YC.isExit("NPC/PlayerData/"+player.getUniqueId()+".yml")==false)
    	{
    ...
    public void NPCNPCconfig(String NPCuuid)
	{
	    YamlManager NPCscript = null;
		YamlController Config_YC = GoldBigDragon_RPG.Main.Main.YC_1;
    	if(Config_YC.isExit("NPC/NPCData/"+NPCuuid+".yml")==false)
    	{
    		NPCscript = Config_YC.getNewConfig("NPC/NPCData/"+NPCuuid+".yml");
    	  	NPCscript.set("NPCuuid", "NPC's uuid");
    	  	NPCscript.set("KoreaLanguage(UTF-8)->JavaEntityLanguage", "http://itpro.cz/juniconv/");
    	  	NPCscript.set("NatureTalk.1.love", 0);
    	  	NPCscript.set("NatureTalk.1.Script", "§f안녕, §e%player%?");
