...
public class PlayerAction
{
	public void PlayerMove(PlayerMoveEvent event)
	{
		...
		String Area = new GoldBigDragon_RPG.ETC.Area().getAreaName(event.getPlayer());
		if(Area != null)
		{
			GoldBigDragon_RPG.ETC.Area A = new GoldBigDragon_RPG.ETC.Area();
			new GoldBigDragon_RPG.ETC.Area().AreaMonsterSpawnAdd(Area, "-1");
		...
		else
		{
			...
			{
				for(int count = 0; count < GoldBigDragon_RPG.ServerTick.ServerTickMain.MobSpawningAreaList.size(); count++)
				{
					if(PrevArea.equals(GoldBigDragon_RPG.ServerTick.ServerTickMain.MobSpawningAreaList.get(count)))
						GoldBigDragon_RPG.ServerTick.ServerTickMain.MobSpawningAreaList.remove(count);
				}
			}
			return;
		}
    	...
    	switch(u.getString(player, (byte)1))
    	{
	    	...
	    	case "CVS"://Calculate Value Set
	    	{
	    		if(u.getInt(player, (byte)1)<=2)
	    		{
					...
					{
		    			int size = QuestConfig.getConfigurationSection(u.getString(player, (byte)2)+".FlowChart").getKeys(false).size();
						sound.SP(player, Sound.ITEM_PICKUP, 1.0F, 1.0F);
    	...
    	case "Hunt":
			if(isIntMinMax(event.getMessage(), player, 1, Integer.MAX_VALUE))
			{
				...
				if(u.getInt(player, (byte)2) != -1)
					Monsternumber = u.getInt(player, (byte)2);
				else
	    	...
	    	else
	    	{
				if(isIntMinMax(event.getMessage(), player, 1, Integer.MAX_VALUE))
				{
    				...
    				if(u.getInt(player, (byte)4) != -1)
    					BlockNumber = u.getInt(player, (byte)4);
    				else
    					BlockNumber = 0;
	...
	private void ItemTypeChatting(PlayerChatEvent event)
	{
			...
			case "HP":
				if(isIntMinMax(Message, player, Integer.MIN_VALUE, Integer.MAX_VALUE))
				{
					...
					if(u.getInt(player, (byte)4) != -1)
					{
						...
						{
							GoldBigDragon_RPG.GUI.UseableItemGUI UGUI = new GoldBigDragon_RPG.GUI.UseableItemGUI();
		...
		ItemList.saveConfig();
		s.SP(player, Sound.HORSE_SADDLE, 1.0F, 1.8F);
		if(u.getType(player) == "UseableItem"
		||u.getType(player).equals("UseableItem"))
			new GoldBigDragon_RPG.GUI.UseableItemGUI().NewUseableItemGUI(player, number);
		else
			IGUI.NewItemGUI(player, number);
		u.clearAll(player);
		return;
			...
			case "AMSM"://AreaMonsterSpawnMonster
				...
				if(answer!=-1)
				{
					if(answer==0)
					{
		    			...
		    			AGUI.AreaMonsterSpawnSettingGUI(player, 0, u.getString(player, (byte)3));
					...
					else
					{
						...
						AGUI.AreaSpawnSpecialMonsterListGUI(player, 0, u.getString(player, (byte)3),u.getString(player, (byte)1));
					}
	    			u.clearAll(player);
				}
				return;
	...
	private void NPCTypeChatting(PlayerChatEvent event)
	{
		...
		YamlController YC_2 = GoldBigDragon_RPG.Main.Main.YC_2;
	  	...
	  	if(YC_2.isExit("NPC/NPCData/"+ NPCuuid +".yml") == false)
	  	{
	  		GoldBigDragon_RPG.Config.NPCconfig NPCC = new GoldBigDragon_RPG.Config.NPCconfig();
	  		NPCC.NPCNPCconfig(NPCuuid);
	  	}
		YamlManager NPCscript = YC_2.getNewConfig("NPC/NPCData/"+ NPCuuid +".yml");
		...
		case "NUC"://NPC'sUpgradeCost
			if(isIntMinMax(Message, player, 0, Integer.MAX_VALUE))
			{
				...
				NGUI.UpgraderGUI(player, 0, u.getString(player, (byte)8));
		...
		case "NPC_TNL"://NPC_TalkNeedLove
			if(isIntMinMax(Message, player, 0, Integer.MAX_VALUE))
			{
				...
				NPGUI.TalkSettingGUI(player, u.getString(player, (byte)2), u.getString(player, (byte)5), Integer.parseInt(u.getString(player, (byte)6)));
				u.clearAll(player);
			}
			return;
		...
		case "NPCJL" ://NPC Job Lord 
			...
			if(isExitJob == true)
			{
				NPCscript = YC_2.getNewConfig("NPC/NPCData/"+ u.getString(player, (byte)2) +".yml");
	...
	private void NewBieTypeChatting(PlayerChatEvent event)
	{
		...
		case "NSM"://NewbieSupportMoney
			if(isIntMinMax(Message, player, 0, Integer.MAX_VALUE))
			{
				YamlManager NewBieYM = YC_2.getNewConfig("ETC/NewBie.yml");
	...
	private void SystemTypeChatting(PlayerChatEvent event)
	{
		...
		case "BMT"://BroadcastMessageTick
			if(isIntMinMax(message, player, 1, 3600))
			{
				...
				Config.saveConfig();
				new GoldBigDragon_RPG.GUI.OPBoxGUI().OPBoxGUI_BroadCast(player, 0);
				u.clearAll(player);
			}
			return;
		...
		case "CSN"://ChangeStatName
			...
			case "CMID": //Change Money ID
			{
				if(isIntMinMax(message, player, 1, Integer.MAX_VALUE))
				{
					if(new GoldBigDragon_RPG.Event.Interact().SetItemDefaultName(Integer.parseInt(message),(byte)0).compareTo("지정되지 않은 아이템")==0)
					{
						player.sendMessage(ChatColor.RED + "[SYSTEM] : 해당 아이템은 존재하지 않습니다!");
		  				s.SP(player, org.bukkit.Sound.ORB_PICKUP, 2.0F, 1.7F);
		  				return;
					...
					case 50:
						...
						GoldBigDragon_RPG.Main.ServerOption.Money1ID = value;
						break;
					case 100:
						Config.set("Server.Money.2.ID",value);
						GoldBigDragon_RPG.Main.ServerOption.Money2ID = value;
						break;
					case 1000:
						Config.set("Server.Money.3.ID",value);
						GoldBigDragon_RPG.Main.ServerOption.Money3ID = value;
						break;
					case 10000:
						Config.set("Server.Money.4.ID",value);
						GoldBigDragon_RPG.Main.ServerOption.Money4ID = value;
						break;
					case 50000:
						Config.set("Server.Money.5.ID",value);
						GoldBigDragon_RPG.Main.ServerOption.Money5ID = value;
						break;
					case -1:
						Config.set("Server.Money.6.ID",value);
						GoldBigDragon_RPG.Main.ServerOption.Money6ID = value;
						break;
					}
					Config.saveConfig();
			...
			case "CMDATA": //Change Money DATA
			{
				if(isIntMinMax(message, player, 0, Integer.MAX_VALUE))
				{
					...
					case 50:
						...
						GoldBigDragon_RPG.Main.ServerOption.Money1DATA = value;
						break;
					case 100:
						Config.set("Server.Money.2.DATA",value);
						GoldBigDragon_RPG.Main.ServerOption.Money2DATA = value;
						break;
					case 1000:
						Config.set("Server.Money.3.DATA",value);
						GoldBigDragon_RPG.Main.ServerOption.Money3DATA = value;
						break;
					case 10000:
						Config.set("Server.Money.4.DATA",value);
						GoldBigDragon_RPG.Main.ServerOption.Money4DATA = value;
						break;
					case 50000:
						Config.set("Server.Money.5.DATA",value);
						GoldBigDragon_RPG.Main.ServerOption.Money5DATA = value;
						break;
					case -1:
						Config.set("Server.Money.6.DATA",value);
						GoldBigDragon_RPG.Main.ServerOption.Money6DATA = value;
						break;
					}
					Config.saveConfig();
					u.clearAll(player);
