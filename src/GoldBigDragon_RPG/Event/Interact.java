	...
	public void PlayerGetItem(PlayerPickupItemEvent event)
	{
		YamlController Event_YC = GoldBigDragon_RPG.Main.Main.YC_1;
	  	if(Event_YC.isExit("Stats/" + event.getPlayer().getUniqueId()+".yml") == false)
	  		new GoldBigDragon_RPG.Config.StatConfig().CreateNewStats(event.getPlayer());
		...
		if((event.getItem().getItemStack().hasItemMeta() == true && event.getItem().getItemStack().getItemMeta().hasDisplayName()))
		{
			...
			if(ItemName.contains(GoldBigDragon_RPG.Main.ServerOption.Money) == true && event.getItem().getItemStack().getEnchantmentLevel(Enchantment.LUCK) == 500)
			{
			  	if(Event_YC.isExit("Stats/" + event.getPlayer().getUniqueId()+".yml") == false)
			  		new GoldBigDragon_RPG.Config.StatConfig().CreateNewStats(event.getPlayer());
		...
		{
			if(ItemName.contains(GoldBigDragon_RPG.Main.ServerOption.Money) == true)
				new GoldBigDragon_RPG.Effect.PacketSender().sendActionBar(event.getPlayer(), ChatColor.GRAY+""+ChatColor.BOLD+""+""+ChatColor.BOLD+ChatColor.BOLD+""+ChatColor.BOLD+"("+""+ChatColor.BOLD+ItemName+""+ChatColor.GRAY+""+ChatColor.BOLD+")");
			else
				new GoldBigDragon_RPG.Effect.PacketSender().sendActionBar(event.getPlayer(), ChatColor.GRAY+""+ChatColor.BOLD+""+""+ChatColor.BOLD+ChatColor.BOLD+""+ChatColor.BOLD+"("+""+ChatColor.BOLD+ItemName+""+" "+ChatColor.GRAY+""+ChatColor.BOLD+event.getItem().getItemStack().getAmount()+"개)");
		}
	  	return;
	}
