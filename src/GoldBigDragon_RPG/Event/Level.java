			...
			if(Main.PartyJoiner.containsKey(player)==false)
			{
				...
				for(;;)
				{
					...
					else
					{
						...
						new GoldBigDragon_RPG.Effect.PacketSender().sendTitleSubTitle(player, "\'"+ChatColor.WHITE+"Level Up!\'", "\'"+ChatColor.WHITE + "레벨 " +ChatColor.YELLOW+ YM.getInt("Stat.Level")+ChatColor.WHITE + "이 되었습니다!\'", 1, 3, 1);
					...
					if(loc.distance(player[count].getLocation()) <= Config.getInt("Party.EXPShareDistance"))
					{
						...
						for(;;)
						{
							...
							else
							{
								...
								YM.set("Stat.StatPoint", YM.getInt("Stat.StatPoint") + (Config.getInt("Server.Level_Up_StatPoint")*Config.getInt("Event.Multiple_Level_Up_StatPoint")));
								YM.saveConfig();
								new GoldBigDragon_RPG.Effect.PacketSender().sendTitleSubTitle(player[count], "\'"+ChatColor.WHITE+"Level Up!\'", "\'"+ChatColor.WHITE + "레벨 " +ChatColor.YELLOW+ YM.getInt("Stat.Level")+ChatColor.WHITE + "이 되었습니다!\'", 1, 3, 1);
