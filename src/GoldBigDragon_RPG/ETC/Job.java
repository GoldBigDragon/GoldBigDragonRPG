...
{	
	...
	//직업 스킬에서 스킬 목록에 등록되지 않은 스킬을 삭제 해 주는 메소드
	{
		...
		if(Config_YC.getNewConfig("config.yml").getBoolean("Server.Like_The_Mabinogi_Online_Stat_System"))
		{
	...
	//마비노기 버전에서는 삭제된 카테고리를 플레이어 스킬 목록에서 제거해 주며, 새로 나온 카테고리를 플레이어에게 등록해 준다.
	{
		...
		if(Config_YC.getNewConfig("config.yml").getBoolean("Server.Like_The_Mabinogi_Online_Stat_System"))
		{
			...
			for(int count=0;count<PlayerList.getConfigurationSection("Mabinogi").getKeys(false).toArray().length;count++)
				PlayerCategori.add(PlayerList.getConfigurationSection("Mabinogi").getKeys(false).toArray()[count].toString());
			for(int count=0;count<JobList.getConfigurationSection("Mabinogi").getKeys(false).toArray().length;count++)
			...
			for(int count = 0; count < Categori.size(); count++)
				if(Categori.get(count).compareTo("Added") != 0)
					if(PlayerCategori.contains(Categori.get(count)) == false)
						PlayerList.set("Mabinogi."+Categori.get(count)+".1",null);
	...
	//플레이어 스킬에서 직업 스킬 목록에 없는 스킬을 삭제 해 주고, 플레이어가 없는 스킬을 추가시켜 주는 메소드
	{
		...
		if(Config_YC.getNewConfig("config.yml").getBoolean("Server.Like_The_Mabinogi_Online_Stat_System") == true)
		{
