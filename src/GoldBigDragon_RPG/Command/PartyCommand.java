...
import GoldBigDragon_RPG.Main.PartyDataObject;
	...
	public void onCommand(CommandSender talker, Command command, String string, String[] args)
    {
	    GoldBigDragon_RPG.Effect.Sound s = new GoldBigDragon_RPG.Effect.Sound();
			...
			if(args.length == 0)
			{
				...
				new GoldBigDragon_RPG.GUI.PartyGUI().PartyGUI_Main(player); return;
			}
			if(args.length <= 1)
			{
				switch(args[0])
				{
					case "목록":
						{
						 	s.SP((Player)talker, org.bukkit.Sound.HORSE_ARMOR, 0.8F, 1.8F);
						 	new GoldBigDragon_RPG.GUI.PartyGUI().PartyListGUI(player, 0);
						}
						return;
					case "탈퇴":
						{
