...
public class AreaCommand extends HelpMessage
{
	public void onCommand(CommandSender talker, Command command, String string, String[] args)
	{
		...
		if(args.length == 2)
		{
			...
			{
			case "생성" :
				if(GoldBigDragon_RPG.Main.Main.catchedLocation1.containsKey(player)==true && GoldBigDragon_RPG.Main.Main.catchedLocation2.containsKey(player) ==true)
				{
					A.CreateNewArea(player, GoldBigDragon_RPG.Main.Main.catchedLocation1.get(player), GoldBigDragon_RPG.Main.Main.catchedLocation2.get(player), args[0]);
					return;
				}
				else
				{
