...
public class UserObject
{
	...
	public boolean addStat_MoneyAndEXP(long Money, long EXP, boolean isAlert)
	{
		...
		{
			if(Money!=0&&EXP!=0)
				new effect.SendPacket().sendActionBar(player, "§b§l[경험치] "+ EXP + " §e§l[§f"+MainServerOption.money+"§e§l] "+ Money);
			else if(Money!=0)
				new effect.SendPacket().sendActionBar(player, "§e§l[§f"+MainServerOption.money+"§e§l] "+ Money);
			else
