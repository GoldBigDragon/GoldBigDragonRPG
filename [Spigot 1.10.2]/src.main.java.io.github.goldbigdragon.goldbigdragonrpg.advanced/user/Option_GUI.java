...
public class Option_GUI extends Util_GUI
{
	public void optionGUI(Player player)
	{
		...
		Stack2("§f옵션", 160,4,1,Arrays.asList("§7기타 설정을 합니다."), 27, inv);
		...
		if(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).isAlert_EXPget()){Stack2("§f§l돈, 경험치 획득 알림", 384,0,1,Arrays.asList("§a[활성화]","§7돈과 경험치 획득을 알립니다."), 2, inv);}
			else{Stack2("§f§l돈, 경험치 획득 알림", 166,0,1,Arrays.asList("§c[비활성화]","§7돈과 경험치 획득을 알리지 않습니다."), 2, inv);}
		if(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).isAlert_ItemPickUp()){Stack2("§f§l아이템 획득 알림", 154,0,1,Arrays.asList("§a[활성화]","§7아이템 획득을 알립니다."), 3, inv);}
		else{Stack2("§f§l아이템 획득 알림", 166,0,1,Arrays.asList("§c[비활성화]","§7아이템 획득을 알리지 않습니다."), 3, inv);}
		if(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).isAlert_MobHealth()){Stack2("§f§l전투 도우미", 381,0,1,Arrays.asList("§a[활성화]","§7각종 전투 상황을 봅니다."), 4, inv);}
		else{Stack2("§f§l전투 도우미", 166,0,1,Arrays.asList("§c[비활성화]","§7각종 전투 상황을 보지 않습니다."), 4, inv);}
		if(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).isAlert_Damage()){Stack2("§f§l데미지 알림", 267,0,1,Arrays.asList("§a[활성화]","§7적에게 입힌 피해를 알립니다."), 5, inv);}
		else{Stack2("§f§l데미지 알림", 166,0,1,Arrays.asList("§c[비활성화]","§7적에게 입힌 피해를 알리지 않습니다."), 5, inv);}
		if(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).isClickUse()){Stack2("§f§l클릭시 사용", 438,0,1,Arrays.asList("§a[활성화]","§7소비 아이템을 클릭시 사용합니다."), 6, inv);}
		else{Stack2("§f§l클릭시 사용", 166,0,1,Arrays.asList("§c[비활성화]","§7소비 아이템을 단축키 처럼 사용합니다."), 6, inv);}
		if(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).isBgmOn()){Stack2("§f§lBGM 재생", 2256,0,1,Arrays.asList("§a[활성화]","§7영역 BGM을 실행 시킵니다."), 14, inv);}
		else{Stack2("§f§lBGM 재생", 166,0,1,Arrays.asList("§c[비활성화]","§7영역 BGM을 듣지 않습니다."), 14, inv);}

		
		if(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).isOption_EquipLook()){Stack2("§f§l장비 구경", 416,0,1,Arrays.asList("§a[허용]","§7다른 플레이어가 자신의 장비를","§7구경할 수 있습니다."), 11, inv);}
		else{Stack2("§f§l장비 구경", 166,0,1,Arrays.asList("§c[비허용]","§7다른 플레이어가 자신의 장비를","§7구경할 수 없습니다."), 11, inv);}

		switch(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).getOption_ChattingType())
		{
		case 0:
			Stack2("§f§l채팅 옵션", 2264,0,1,Arrays.asList("§f[일반]","§7일반적인 채팅을 합니다."), 12, inv);
			break;
		case 1:
			Stack2("§f§l채팅 옵션", 2261,0,1,Arrays.asList("§9[파티]","§7파티 채팅을 합니다."), 12, inv);
			break;
		case 2:
			Stack2("§f§l채팅 옵션", 2260,0,1,Arrays.asList("§a[무음]","§7욕 하고 싶으나 용기가 나지 않을때...."), 12, inv);
			break;
		case 3:
			Stack2("§f§l채팅 옵션", 2258,0,1,Arrays.asList("§d[관리자]","§7관리자 끼리의 채팅을 합니다.","§c※ 일반 유저는 사용할 수 없습니다."), 12, inv);
			break;
		}
		if(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).isOption_HotBarSound()){Stack2("§f§l장비 전환 사운드", 307,0,1,Arrays.asList("§a[허용]","§7핫바를 움직일 때 마다 소리가 납니다."), 13, inv);}
		else{Stack2("§f§l장비 전환 사운드", 166,0,1,Arrays.asList("§c[비허용]","§7핫바를 움직여도 소리가 나지 않습니다."), 13, inv);}

		if(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).isOption_SeeInventory()){Stack2("§f§l우 클릭시 상대방 정보 보기", 307,0,1,Arrays.asList("§a[허용]","§7대상을 우 클릭하면 정보창이 뜹니다."), 15, inv);}
		else{Stack2("§f§l우 클릭시 상대방 정보 보기", 166,0,1,Arrays.asList("§c[비허용]","§7대상을 우 클릭하여도 정보창이 뜨지 않습니다."), 15, inv);}
