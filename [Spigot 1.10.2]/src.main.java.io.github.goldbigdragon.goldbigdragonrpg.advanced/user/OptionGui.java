...
public class OptionGui extends UtilGui
{
	public void optionGUI(Player player)
	{
		...
		if(main.MainServerOption.PlayerList.get(player.getUniqueId().toString()).isAlert_EXPget()){Stack2("§f§l돈, 경험치 획득 알림", 384,0,1,Arrays.asList("§a[활성화]","§7돈과 경험치 획득을 알립니다."), 2, inv);}
			else{Stack2("§f§l돈, 경험치 획득 알림", 166,0,1,Arrays.asList("§c[비활성화]","§7돈과 경험치 획득을 알리지 않습니다."), 2, inv);}
		if(main.MainServerOption.PlayerList.get(player.getUniqueId().toString()).isAlert_ItemPickUp()){Stack2("§f§l아이템 획득 알림", 154,0,1,Arrays.asList("§a[활성화]","§7아이템 획득을 알립니다."), 3, inv);}
		else{Stack2("§f§l아이템 획득 알림", 166,0,1,Arrays.asList("§c[비활성화]","§7아이템 획득을 알리지 않습니다."), 3, inv);}
		if(main.MainServerOption.PlayerList.get(player.getUniqueId().toString()).isAlert_MobHealth()){Stack2("§f§l전투 도우미", 381,0,1,Arrays.asList("§a[활성화]","§7각종 전투 상황을 봅니다."), 4, inv);}
		else{Stack2("§f§l전투 도우미", 166,0,1,Arrays.asList("§c[비활성화]","§7각종 전투 상황을 보지 않습니다."), 4, inv);}
		if(main.MainServerOption.PlayerList.get(player.getUniqueId().toString()).isAlert_Damage()){Stack2("§f§l데미지 알림", 267,0,1,Arrays.asList("§a[활성화]","§7적에게 입힌 피해를 알립니다."), 5, inv);}
		else{Stack2("§f§l데미지 알림", 166,0,1,Arrays.asList("§c[비활성화]","§7적에게 입힌 피해를 알리지 않습니다."), 5, inv);}
		if(main.MainServerOption.PlayerList.get(player.getUniqueId().toString()).isClickUse()){Stack2("§f§l클릭시 사용", 438,0,1,Arrays.asList("§a[활성화]","§7소비 아이템을 클릭시 사용합니다."), 6, inv);}
		else{Stack2("§f§l클릭시 사용", 166,0,1,Arrays.asList("§c[비활성화]","§7소비 아이템을 단축키 처럼 사용합니다."), 6, inv);}
		if(main.MainServerOption.PlayerList.get(player.getUniqueId().toString()).isBgmOn()){Stack2("§f§lBGM 재생", 2256,0,1,Arrays.asList("§a[활성화]","§7영역 BGM을 실행 시킵니다."), 14, inv);}
		else{Stack2("§f§lBGM 재생", 166,0,1,Arrays.asList("§c[비활성화]","§7영역 BGM을 듣지 않습니다."), 14, inv);}

		
		if(main.MainServerOption.PlayerList.get(player.getUniqueId().toString()).isOption_EquipLook()){Stack2("§f§l장비 구경", 416,0,1,Arrays.asList("§a[허용]","§7다른 플레이어가 자신의 장비를","§7구경할 수 있습니다."), 11, inv);}
		...
		if(main.MainServerOption.PlayerList.get(player.getUniqueId().toString()).isOption_HotBarSound()){Stack2("§f§l장비 전환 사운드", 307,0,1,Arrays.asList("§a[허용]","§7핫바를 움직일 때 마다 소리가 납니다."), 13, inv);}
		else{Stack2("§f§l장비 전환 사운드", 166,0,1,Arrays.asList("§c[비허용]","§7핫바를 움직여도 소리가 나지 않습니다."), 13, inv);}

		if(main.MainServerOption.PlayerList.get(player.getUniqueId().toString()).isOption_SeeInventory()){Stack2("§f§l우 클릭시 상대방 정보 보기", 307,0,1,Arrays.asList("§a[허용]","§7대상을 우 클릭하면 정보창이 뜹니다."), 15, inv);}
