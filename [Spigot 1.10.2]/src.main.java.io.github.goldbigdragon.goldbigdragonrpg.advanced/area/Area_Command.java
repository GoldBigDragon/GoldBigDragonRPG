...
public class Area_Command
{
	public void onCommand(CommandSender talker, Command command, String string, String[] args)
	{
		...
		if(args.length == 2)
		{
			area.Area_Main areaMain = new area.Area_Main();
			switch(args[1])
			{
			case "생성" :
				if(main.Main_ServerOption.catchedLocation1.containsKey(player)&&main.Main_ServerOption.catchedLocation2.containsKey(player))
				{
					areaMain.CreateNewArea(player, main.Main_ServerOption.catchedLocation1.get(player), main.Main_ServerOption.catchedLocation2.get(player), args[0]);
					return;
				}
				else
				{
		...
		else
		{
			...
			for(int a =2; a<= ((args.length)-1);a++)
				tempString = tempString+args[a]+" ";
			switch(args[1])
			{
			case "이름" :
				areaMain.OptionSetting(player, args[0],(char) 0, tempString);
				return;
			case "설명" :
				areaMain.OptionSetting(player, args[0],(char) 1, tempString);
				return;
			}
		}
	}
