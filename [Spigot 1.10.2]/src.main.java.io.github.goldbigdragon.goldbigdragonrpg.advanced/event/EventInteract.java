			...
			if(player.getInventory().getItemInMainHand().getItemMeta().hasLore()==true)
			{
				   ...
				   LoreString.contains("[돈]"))
				{
					...
					if(LoreString.contains("[소비]"))
						new UseableItemMain().useAbleItemUse(player, "소비");
					else if(LoreString.contains("[귀환서]"))
						new UseableItemMain().useAbleItemUse(player, "귀환서");
					else if(LoreString.contains("[주문서]"))
						new UseableItemMain().useAbleItemUse(player, "주문서");
					else if(LoreString.contains("[스킬북]"))
						new UseableItemMain().useAbleItemUse(player, "스킬북");
					else if(LoreString.contains("[돈]"))
						new UseableItemMain().useAbleItemUse(player, "돈");
					return;
				}
			}
		}
