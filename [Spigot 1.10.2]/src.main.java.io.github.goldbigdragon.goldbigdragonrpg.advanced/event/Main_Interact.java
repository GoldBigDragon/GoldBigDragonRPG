...
import customitem.UseableItem_Main;
...
public class Main_Interact
{
	...
	public void PlayerInteract(PlayerInteractEvent event)
	{
		...
		{
			if(new corpse.Corpse_Main().DeathCapture(event.getPlayer(),false))
				return;
			...
			{
				new dungeon.Dungeon_Main().DungeonInteract(event);
				return;
			}
	...
	public void PlayerInteractEntity (PlayerInteractEntityEvent event)
	{
		if(event.getHand()==EquipmentSlot.HAND)
		{
			...
			{
				if(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).isOption_SeeInventory())
				{
					Player t = (Player)target;
					if(t.isOnline()==true)
					{
						user.Equip_GUI EGUI = new user.Equip_GUI();
		    ...
		    if(player.isOp())
		    {
		    	...
		    	if(Type!=null)
		    		if(new UserData_Object().getType(player).compareTo("Quest")==0)
		    			new quest.Quest_InteractEvent().EntityInteract(event, Type);
		    }

			String[] Area = new area.Area_Main().getAreaName(target);
			if(Area != null)
			{
				if(new area.Area_Main().getAreaOption(Area[0], (char) 7) == false && event.getPlayer().isOp() == false)
				{
					event.setCancelled(true);
					SoundEffect.SP(event.getPlayer(), org.bukkit.Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 2.0F, 1.7F);
	...
	private void OPwork(PlayerInteractEvent event)
	{
		...
		if(u.getType(player) != null)
		{
			if(u.getType(player).compareTo("Quest")==0)
				new quest.Quest_InteractEvent().BlockInteract(event);
	...
	private void ItemUse(PlayerInteractEvent event)
	{
		...
		if(player.getInventory().getItemInMainHand().hasItemMeta() == true)
		{
			if(player.getInventory().getItemInMainHand().getItemMeta().hasLore()==true)
			{
				...
				{
	    			if(main.Main_ServerOption.MonsterList.containsKey(ChatColor.stripColor(event.getItem().getItemMeta().getDisplayName())))
	    			{
	    				if(player.isOp())
	    					new monster.Monster_Spawn().SpawnMob(event.getClickedBlock().getLocation(), ChatColor.stripColor(event.getItem().getItemMeta().getDisplayName()), (byte)-1, null, (char) -1, false);
	    				else
	    				{
					...
					for(int counter = 0; counter < lore.length; counter ++)
						if(lore[counter].toString().contains("내용") == true)
						{
							...
							if(player.isOp())
							{
								...
								main.Main_ServerOption.Mapping = true;
								return;
							}
						}
				}
	...
	private void SlotMachine(PlayerInteractEvent event)
	{
		...
		{
			event.setCancelled(true);
			new admin.Gamble_GUI().SlotMachine_PlayGUI(event.getPlayer(), BlockLocation);
		}
		return;
	}
	...
	private void OPwork_Gamble(PlayerInteractEvent event)
	{
		...
		else if(event.getAction()==Action.RIGHT_CLICK_BLOCK)
		{
			if(u.getString(player, (byte)0).compareTo("NSM")==0)//NewSlotMachine
			{
				...
				new admin.Gamble_GUI().SlotMachine_DetailGUI(player, Name);
			}
		}
		return;
	}
	...
	private void AreaChecker(PlayerInteractEvent event)
	{
		...
		if(player.getInventory().getItemInMainHand().getTypeId() == configYaml.getInt("Server.AreaSettingWand"))
		{
			...
			if(event.getAction()==Action.LEFT_CLICK_BLOCK)
			{
				main.Main_ServerOption.catchedLocation1.put(player, block.getLocation());
			...
			{
				main.Main_ServerOption.catchedLocation2.put(player, event.getClickedBlock().getLocation());
	...
	{
	  	if(main.Main_ServerOption.PlayerList.get(event.getPlayer().getUniqueId().toString()).isAlert_ItemPickUp())
	  	{
			String ItemName = null;
	  		...
	  		new effect.SendPacket().sendActionBar(event.getPlayer(), "§7§l("+""+ChatColor.BOLD+ItemName+""+" "+ChatColor.GRAY+""+ChatColor.BOLD+event.getItem().getItemStack().getAmount()+"개)");
	  	}
	  	return;
	}
	...
	public String SetItemDefaultName(short itemCode,byte itemData)
	{
		...
		switch (itemCode)
		{
			...
			case 137: return "§c§l커"+""+ChatColor.BOLD+ChatColor.GOLD+""+ChatColor.BOLD+"맨"+""+ChatColor.BOLD+"§e§l드"+""+ChatColor.BOLD+ChatColor.DARK_GREEN+""+ChatColor.BOLD+" 블"+""+ChatColor.BOLD+"§9§l록";
			...
			case 2256: return "§7§l[음반] [곡 명 : "+""+ChatColor.BOLD+"§e§l13"+""+ChatColor.BOLD+"§7§l]";
			case 2257: return "§7§l[음반] [곡 명 : "+""+ChatColor.BOLD+"§a§lcat"+""+ChatColor.BOLD+"§7§l]";
			case 2258: return "§7§l[음반] [곡 명 : "+""+ChatColor.BOLD+"§c§lblocks"+""+ChatColor.BOLD+"§7§l]";
			case 2259: return "§7§l[음반] [곡 명 : "+""+ChatColor.BOLD+ChatColor.DARK_RED+""+ChatColor.BOLD+"chirp"+""+ChatColor.BOLD+"§7§l]";
			case 2260: return "§7§l[음반] [곡 명 : "+""+ChatColor.BOLD+"§a§lfar"+""+ChatColor.BOLD+"§7§l]";
			case 2261: return "§7§l[음반] [곡 명 : "+""+ChatColor.BOLD+ChatColor.MAGIC+""+ChatColor.BOLD+"mall"+""+ChatColor.BOLD+"§7§l]";
			case 2262: return "§7§l[음반] [곡 명 : "+""+ChatColor.BOLD+ChatColor.DARK_PURPLE+""+ChatColor.BOLD+"mellohi"+""+ChatColor.BOLD+"§7§l]";
			case 2263: return "§7§l[음반] [곡 명 : "+""+ChatColor.BOLD+ChatColor.BLACK+""+ChatColor.BOLD+"stal"+""+ChatColor.BOLD+"§7§l]";
			case 2264: return "§7§l[음반] [곡 명 : "+""+ChatColor.BOLD+"§f§lstrad"+""+ChatColor.BOLD+"§7§l]";
			case 2265: return "§7§l[음반] [곡 명 : "+""+ChatColor.BOLD+ChatColor.DARK_GREEN+""+ChatColor.BOLD+"ward"+""+ChatColor.BOLD+"§7§l]";
			case 2266: return "§7§l[음반] [곡 명 : "+""+ChatColor.BOLD+ChatColor.BLACK+""+ChatColor.BOLD+"11"+""+ChatColor.BOLD+"§7§l]";
			case 2267: return "§7§l[음반] [곡 명 : "+""+ChatColor.BOLD+"§b§lwait"+""+ChatColor.BOLD+"§7§l]";
		}
		return name;
	}
}
