...
public class Main_Interact
{
	...
	public void PlayerInteractEntity (PlayerInteractEntityEvent event)
	{
		if(event.getHand()==EquipmentSlot.HAND)
		{
		    ...
		    if(player.isOp())
		    {
		    	...
		    	if(Type!=null)
		    		if(new UserData_Object().getType(player).equals("Quest"))
	...
	private void SlotMachine(PlayerInteractEvent event)
	{
		...
		{
			event.setCancelled(true);
			new admin.Gamble_GUI().slotMachinePlayGui(event.getPlayer(), BlockLocation);
		}
		return;
	}
	...
	private void OPwork_Gamble(PlayerInteractEvent event)
	{
		...
		else if(event.getAction()==Action.RIGHT_CLICK_BLOCK)
		{
			if(u.getString(player, (byte)0).equals("NSM"))//NewSlotMachine
			{
				...
				new admin.Gamble_GUI().slotMachineDetailGUI(player, Name);
			}
		}
		return;
	}
	...
	public void PlayerGetItem(PlayerPickupItemEvent event)
	{
	  	if(main.Main_ServerOption.PlayerList.get(event.getPlayer().getUniqueId().toString()).isAlert_ItemPickUp())
	  	{
			String ItemName = null;
	  		...
	  		new effect.SendPacket().sendActionBar(event.getPlayer(), "§7§l(§l"+ItemName+" §7§l"+event.getItem().getItemStack().getAmount()+"개)");
	  	}
	  	return;
	}
