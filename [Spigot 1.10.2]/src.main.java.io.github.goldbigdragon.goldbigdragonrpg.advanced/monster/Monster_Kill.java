...
public class Monster_Kill
{
	...
	public String getRealName(Entity entity)
	{
		...
		if(name.length()<=0)
			...
			return main.Main_ServerOption.MonsterNameMatching.get(name);
	}
	...
	public void Boomb(Entity entity)
	{
		...
		int MonsterINT = 10;
		int radius = 5;
		if(main.Main_ServerOption.MonsterList.containsKey(EntityName))
			MonsterINT = main.Main_ServerOption.MonsterList.get(EntityName).getINT();
		else
		{
		        ...
		        if(Choosedentity.isDead()==false)
		        {
			        ...
			        if(Choosedentity.getType()==EntityType.PLAYER)
			        {
			        	...
			        	if(p.getGameMode()==GameMode.SURVIVAL||p.getGameMode()==GameMode.ADVENTURE)
			        	{
			        		if(p.isOnline())
			        		{
			        			DEF = main.Main_ServerOption.PlayerList.get(p.getUniqueId().toString()).getStat_DEF();
			        			PRO = main.Main_ServerOption.PlayerList.get(p.getUniqueId().toString()).getStat_Protect();
			        		}
			        		else if(name!=null)
			        		{
								name = getRealName(Choosedentity);
			        			DEF = main.Main_ServerOption.MonsterList.get(name).getDEF();
			        			PRO = main.Main_ServerOption.MonsterList.get(name).getPRO();
			        		}
			        	}
			        }
			        else if(name!=null)
					...
					name.charAt(4)=='¡×'&&name.charAt(5)=='2')
				{
					...
					case 'e' : //ÀÏ¹Ý
						if(SearchRoomMonster((byte) 20, name.charAt(9), loc) <= 0)
							new dungeon.Dungeon_Main().DungeonTrapDoorWorker(loc, false);
						break;
					case '1' : //´ÙÀ½ ¿þÀÌºê Á¸Àç
						if(SearchRoomMonster((byte) 20, name.charAt(9), loc) <= 0)
							new dungeon.Dungeon_Main().MonsterSpawn(loc);
						break;
					case '4' : //¿­¼è °¡Áø ³ð
						if(SearchRoomMonster((byte) 20, name.charAt(9), loc) <= 0)
							new dungeon.Dungeon_Main().DungeonTrapDoorWorker(loc, false);
						...
						ItemMeta im = item.getItemMeta();
						im.setDisplayName(ChatColor.GREEN+""+ChatColor.BLACK+""+ChatColor.GREEN+""+"¡×f¡×l[´øÀü ·ë ¿­¼è]");
					...
					case '2' : //º¸½º [º¸½º¹æ ¹®À» Å½ÁöÇÏ±â À§ÇØ¼­ º¸»ó¹æ Ã¶Ã¢ Áß¾ÓÀÇ À§Ä¡¸¦ ´øÀü ÄÜÇÇ±×¿¡ ÀúÀå ½ÃÅ²´Ù.]
						...
						if(name != null)
						{
							...
							if(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).getDungeon_Enter() != null && main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).getDungeon_UTC() > 1)
							{
								dungeonYaml.getConfig("Dungeon/Dungeon/"+main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).getDungeon_Enter()+"/Entered/"+main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).getDungeon_UTC()+".yml");
								if(dungeonYaml.contains("Boss"))
								{
									...
									if(BossList.isEmpty() == false)
									{
										...
										new dungeon.Dungeon_Main().DungeonClear(player, loc);
								}
							}
						}
						break;
    				...
    				|| event.getEntity().getLastDamageCause().getCause() == DamageCause.MAGIC)
    		{
				if(Bukkit.getServer().getPlayer(event.getEntity().getKiller().getName()).isOnline() == true)
				{
					...
					if(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).isAlert_MobHealth())
					    new SendPacket().sendTitleSubTitle(player, "\'" +ChatColor.BLACK+"¡á¡á¡á¡á¡á¡á¡á¡á¡á¡á"+ "\'", "\'" +ChatColor.DARK_RED+""+ChatColor.BOLD+"[DEAD]"+ "\'", (byte)0, (byte)0, (byte)1);
    				Reward(event,player);
    				Quest(event, player);
					return;
				}
	...
	public void Reward(EntityDeathEvent event, Player player)
	{
		...
		if(40 <= N.RandomNum(0, 100) * Main_ServerOption.Event_DropChance)
		{
			...
			if(lucky >= N.RandomNum(0, 100))
			{
				...
				if(luckysize <= 80){player.sendMessage(ChatColor.YELLOW +""+ChatColor.BOLD+ "[SYSTEM] : ·°Å° ÇÇ´Ï½Ã!");amount = 2;	SoundEffect.SP(player, org.bukkit.Sound.ENTITY_PLAYER_LEVELUP, 0.5F, 0.9F);}
				else if(luckysize <= 95){player.sendMessage(ChatColor.YELLOW +""+ChatColor.BOLD+ "[SYSTEM] : ºò ·°Å° ÇÇ´Ï½Ã!");amount = 5;	SoundEffect.SP(player, org.bukkit.Sound.ENTITY_PLAYER_LEVELUP, 0.7F, 1.0F);}
				else{player.sendMessage(ChatColor.YELLOW +""+ChatColor.BOLD+ "[SYSTEM] : ÈÞÁî ·°Å° ÇÇ´Ï½Ã!");amount = 20;	SoundEffect.SP(player, org.bukkit.Sound.ENTITY_PLAYER_LEVELUP, 1.0F, 1.1F);}
			}
		}
		else
			amount = 0;
		...
		{
			if(amount == 0)
				new util.Util_Player().addMoneyAndEXP(player, 0, main.Main_ServerOption.MonsterList.get(name).getEXP(), event.getEntity().getLocation(), true, false);
			else
				new util.Util_Player().addMoneyAndEXP(player, amount* N.RandomNum(main.Main_ServerOption.MonsterList.get(name).getMinMoney(), main.Main_ServerOption.MonsterList.get(name).getMaxMoney()), main.Main_ServerOption.MonsterList.get(name).getEXP(), event.getEntity().getLocation(), true, false);
			return;
		}
		else
		{
			...
			if(ET == EntityType.SKELETON)
			{
				...
				if(s.getSkeletonType() == SkeletonType.NORMAL)
					new util.Util_Player().addMoneyAndEXP(player, N.RandomNum(configYaml.getInt("Normal_Monster.SKELETON.MIN_MONEY"), configYaml.getInt("Normal_Monster.SKELETON.MAX_MONEY"))*amount, configYaml.getLong("Normal_Monster.SKELETON.EXP"), event.getEntity().getLocation(), true, false);
				else
					new util.Util_Player().addMoneyAndEXP(player, N.RandomNum(configYaml.getInt("Normal_Monster.NETHER_SKELETON.MIN_MONEY"), configYaml.getInt("Normal_Monster.NETHER_SKELETON.MAX_MONEY"))*amount, configYaml.getLong("Normal_Monster.NETHER_SKELETON.EXP"), event.getEntity().getLocation(), true, false);
			}
			else if(ET == EntityType.CREEPER)
			{
				Creeper c = (Creeper)event.getEntity();
				if(c.isPowered() == false)
					new util.Util_Player().addMoneyAndEXP(player, N.RandomNum(configYaml.getInt("Normal_Monster.CREEPER.MIN_MONEY"), configYaml.getInt("Normal_Monster.CREEPER.MAX_MONEY"))*amount, configYaml.getLong("Normal_Monster.CREEPER.EXP"), event.getEntity().getLocation(), true, false);
				else
					new util.Util_Player().addMoneyAndEXP(player, N.RandomNum(configYaml.getInt("Normal_Monster.CHARGED_CREEPER.MIN_MONEY"), configYaml.getInt("Normal_Monster.CHARGED_CREEPER.MAX_MONEY"))*amount, configYaml.getLong("Normal_Monster.CHARGED_CREEPER.EXP"), event.getEntity().getLocation(), true, false);
			}
			else if(ET == EntityType.SLIME)
			{
				Slime sl = (Slime)event.getEntity();
				if(sl.getSize() == 1)
					new util.Util_Player().addMoneyAndEXP(player, N.RandomNum(configYaml.getInt("Normal_Monster.SLIME_SMALL.MIN_MONEY"), configYaml.getInt("Normal_Monster.SLIME_SMALL.MAX_MONEY"))*amount, configYaml.getLong("Normal_Monster.SLIME_SMALL.EXP"), event.getEntity().getLocation(), true, false);
				else if(sl.getSize() <= 3)
					new util.Util_Player().addMoneyAndEXP(player, N.RandomNum(configYaml.getInt("Normal_Monster.SLIME_MIDDLE.MIN_MONEY"), configYaml.getInt("Normal_Monster.SLIME_MIDDLE.MAX_MONEY"))*amount, configYaml.getLong("Normal_Monster.SLIME_MIDDLE.EXP"), event.getEntity().getLocation(), true, false);
				else if(sl.getSize() == 4)
					new util.Util_Player().addMoneyAndEXP(player, N.RandomNum(configYaml.getInt("Normal_Monster.SLIME_BIG.MIN_MONEY"), configYaml.getInt("Normal_Monster.SLIME_BIG.MAX_MONEY"))*amount, configYaml.getLong("Normal_Monster.SLIME_BIG.EXP"), event.getEntity().getLocation(), true, false);
				else
					new util.Util_Player().addMoneyAndEXP(player, N.RandomNum(configYaml.getInt("Normal_Monster.SLIME_HUGE.MIN_MONEY"), configYaml.getInt("Normal_Monster.SLIME_HUGE.MAX_MONEY"))*amount, configYaml.getLong("Normal_Monster.SLIME_HUGE.EXP"), event.getEntity().getLocation(), true, false);
			}
			else if(ET == EntityType.MAGMA_CUBE)
			{
				MagmaCube ma = (MagmaCube)event.getEntity();
				if(ma.getSize() == 1)
					new util.Util_Player().addMoneyAndEXP(player, N.RandomNum(configYaml.getInt("Normal_Monster.MAGMA_CUBE_SMALL.MIN_MONEY"), configYaml.getInt("Normal_Monster.MAGMA_CUBE_SMALL.MAX_MONEY"))*amount, configYaml.getLong("Normal_Monster.MAGMA_CUBE_SMALL.EXP"), event.getEntity().getLocation(), true, false);
				else if(ma.getSize() <= 3)
					new util.Util_Player().addMoneyAndEXP(player, N.RandomNum(configYaml.getInt("Normal_Monster.MAGMA_CUBE_MIDDLE.MIN_MONEY"), configYaml.getInt("Normal_Monster.MAGMA_CUBE_MIDDLE.MAX_MONEY"))*amount, configYaml.getLong("Normal_Monster.MAGMA_CUBE_MIDDLE.EXP"), event.getEntity().getLocation(), true, false);
				else if(ma.getSize() == 4)
					new util.Util_Player().addMoneyAndEXP(player, N.RandomNum(configYaml.getInt("Normal_Monster.MAGMA_CUBE_BIG.MIN_MONEY"), configYaml.getInt("Normal_Monster.MAGMA_CUBE_BIG.MAX_MONEY"))*amount, configYaml.getLong("Normal_Monster.MAGMA_CUBE_BIG.EXP"), event.getEntity().getLocation(), true, false);
				else
					new util.Util_Player().addMoneyAndEXP(player, N.RandomNum(configYaml.getInt("Normal_Monster.MAGMA_CUBE_HUGE.MIN_MONEY"), configYaml.getInt("Normal_Monster.MAGMA_CUBE_HUGE.MAX_MONEY"))*amount, configYaml.getLong("Normal_Monster.MAGMA_CUBE_HUGE.EXP"), event.getEntity().getLocation(), true, false);
			}
			else
			{
				if(configYaml.contains("Normal_Monster."+ET.toString()))
					new util.Util_Player().addMoneyAndEXP(player, N.RandomNum(configYaml.getInt("Normal_Monster."+ET.toString()+".MIN_MONEY"), configYaml.getInt("Normal_Monster."+ET.toString()+".MAX_MONEY"))*amount, configYaml.getLong("Normal_Monster."+ET.toString()+".EXP"), event.getEntity().getLocation(), true, false);
			}
		}
		return;
	}
