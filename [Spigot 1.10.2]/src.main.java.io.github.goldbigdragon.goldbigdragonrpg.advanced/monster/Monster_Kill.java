    				...
    				|| event.getEntity().getLastDamageCause().getCause() == DamageCause.MAGIC)
    		{
				if(Bukkit.getServer().getPlayer(event.getEntity().getKiller().getName()).isOnline() == true)
				{
					...
					if(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).isAlert_MobHealth())
					    new SendPacket().sendTitleSubTitle(player, "\'¡×0¡á¡á¡á¡á¡á¡á¡á¡á¡á¡á\'", "\'¡×4¡×l[DEAD]\'", (byte)0, (byte)0, (byte)1);
    				Reward(event,player);
    				Quest(event, player);
					return;
				}
	...
	public void Reward(EntityDeathEvent event, Player player)
	{
		...
		if(40 <= N.RandomNum(0, 100) * Main_ServerOption.eventDropChance)
		{
			...
			if(lucky >= N.RandomNum(0, 100))
			{
				...
				if(luckysize <= 80){player.sendMessage("¡×e¡×l[SYSTEM] : ·°Å° ÇÇ´Ï½Ã!");amount = 2;	SoundEffect.SP(player, org.bukkit.Sound.ENTITY_PLAYER_LEVELUP, 0.5F, 0.9F);}
				else if(luckysize <= 95){player.sendMessage("¡×e¡×l[SYSTEM] : ºò ·°Å° ÇÇ´Ï½Ã!");amount = 5;	SoundEffect.SP(player, org.bukkit.Sound.ENTITY_PLAYER_LEVELUP, 0.7F, 1.0F);}
				else{player.sendMessage("¡×e¡×l[SYSTEM] : ÈÞÁî ·°Å° ÇÇ´Ï½Ã!");amount = 20;	SoundEffect.SP(player, org.bukkit.Sound.ENTITY_PLAYER_LEVELUP, 1.0F, 1.1F);}
			}
		}
		else
			amount = 0;
