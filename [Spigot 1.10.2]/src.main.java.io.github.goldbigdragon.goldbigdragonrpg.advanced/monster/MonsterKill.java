		...
		else
		{
			...
			if(ET == EntityType.SKELETON)
			{
				...
				if(s.getSkeletonType() == SkeletonType.NORMAL)
					...
					new util.UtilPlayer().addMoneyAndEXP(player, N.RandomNum(configYaml.getInt("Normal_Monster.NETHER_SKELETON.MIN_MONEY"), configYaml.getInt("Normal_Monster.NETHER_SKELETON.MAX_MONEY"))*amount, configYaml.getLong("Normal_Monster.NETHER_SKELETON.EXP"), event.getEntity().getLocation(), true, false);
			}
