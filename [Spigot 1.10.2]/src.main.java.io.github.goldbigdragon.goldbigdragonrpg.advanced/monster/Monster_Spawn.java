...
import ai_creature.CreatureBlaze;
import ai_creature.CreatureCaveSpider;
import ai_creature.CreatureCreeper;
import ai_creature.CreatureEnderMite;
import ai_creature.CreatureEnderman;
import ai_creature.CreatureGiant;
import ai_creature.CreatureGuardian;
import ai_creature.CreatureMonster;
import ai_creature.CreaturePigZombie;
import ai_creature.CreaturePolarBear;
import ai_creature.CreatureSilverFish;
import ai_creature.CreatureSkeleton;
import ai_creature.CreatureSpider;
import ai_creature.CreatureWitch;
import ai_creature.CreatureWither;
import ai_creature.CreatureZombie;
import ai_monster.NMSUtils;
...
public class Monster_Spawn
{
	public void EntitySpawn(CreatureSpawnEvent event)
	{
		...
		if(event.getSpawnReason() == SpawnReason.SLIME_SPLIT)
		{
			...
			{
				if(main.Main_ServerOption.MonsterNameMatching.containsKey(event.getEntity().getCustomName()))
				{
					event.setCancelled(true);
				}
			}
		...
		if(Area != null)
		{
			...
			else if(A.getAreaOption(Area[0], (char) 8) == false)
			{
				...
				for(int count = 0;count<10;count++)
				{
					...
					{
						short RandomMob = (short) new util.Util_Number().RandomNum(0, MobNameList.length-1);
						if(main.Main_ServerOption.MonsterList.containsKey(MobNameList[RandomMob].toString()))
						{
							new monster.Monster_Spawn().SpawnMob(event.getLocation(), MobNameList[RandomMob].toString(), (byte) -1, null,(char) -1, false);
							isExit = true;
						}
						else
						{
		...
		new monster.Monster_Spawn().SpawnEffect(event.getEntity(),event.getLocation(), (byte) configYaml.getInt("Server.MonsterSpawnEffect"));
		return;
	}
	...
	private LivingEntity getEntity(LivingEntity Monster, String mob, byte DungeonSpawning, int[] XYZloc, char Group)
	{
		if(mob!=null)
		{
			...
			{
				if(monsterYaml.getString(mob+".Type").equalsIgnoreCase("네더스켈레톤") == true)
				{
			...
			else if(Monster.getType() == EntityType.SLIME)
			{
				switch(monsterYaml.getString(mob + ".Type"))
				{
					...
					default : ((Slime) Monster).setSize(new util.Util_Number().RandomNum(1, 4));break;
				}
			}
			else if(Monster.getType() == EntityType.MAGMA_CUBE)
			{
				switch(monsterYaml.getString(mob + ".Type"))
				{
					...
					default : ((MagmaCube) Monster).setSize(new util.Util_Number().RandomNum(1, 4));break;
				}
			}
