...
public class Corpse_GUI extends Util_GUI
{
	...
	public void OpenReviveSelectGUI(Player player)
	{
		if(!player.getOpenInventory().getTitle().equals("§0§0§9§0§0§r§0행동불능 상태 -부활 방법 선택-"))
		{
			...
			Inventory inv = Bukkit.createInventory(null, 27, UniqueCode + "§0행동불능 상태 -부활 방법 선택-");
		  	...
		  	try
		  	{
				if(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).getETC_LastVisited()==null||main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).getETC_LastVisited().equals("null"))
					...
					Stack2("§6§l[가까운 마을에서 부활]", 345,0,1,Arrays.asList("§e"+ main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).getETC_LastVisited()+"§7에서 부활합니다.","","§a + "+configYaml.getString("Death.Spawn_Home.SetHealth")+" 생명력","§c - 경험치 "+configYaml.getString("Death.Spawn_Home.PenaltyEXP")+" 감소","§c - 소지금 "+configYaml.getString("Death.Spawn_Home.PenaltyMoney")+" 감소"), 10, inv);
		  	}
	...
	public void ReviveAtLastVisitedArea(Player player)
	{
		...
		{
			if(main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).getETC_LastVisited().equals("null"))
