...
public class Dungeon_ServerTask
{
			...
			else
			{
		    	...
		    	case '◇' : 
			    		...
			    		if(GridImage=='◇')
			    		{
			                ...
			                for(char c=0;c<4;c++)
			                	randomchest[c]=146;
			                byte randomnum = (byte) new util.Util_Number().RandomNum(0, 3);
	...
	public void CreateKeyRoom(Dungeon_ScheduleObject DSO)
	{
    	...
    	case '▷' : 
    	case '♥' : 
			byte color = (byte) new util.Util_Number().RandomNum(1, 5);
			switch(color)
			{
    	...
    	case '♥' : 
    		if(KeyGridImage=='♠'||KeyGridImage=='♥')
    		{
    			...
    			dungeonPlayerYaml.getConfig("Dungeon/Dungeon/"+DSO.getDungeonName()+"/Entered/"+DSO.getUTC()+".yml");
    			dungeonPlayerYaml.set("BossRoomDoor.X", BossRoomDoorLoc.getX());
    			dungeonPlayerYaml.set("BossRoomDoor.Y", BossRoomDoorLoc.getY());
    			dungeonPlayerYaml.set("BossRoomDoor.Z", BossRoomDoorLoc.getZ());
    			dungeonPlayerYaml.saveConfig();
    		}
    	...
    	else
    	{
    		...
    		dungeonOptionYaml.getConfig("Dungeon/Dungeon/"+DSO.getDungeonName()+"/Option.yml");
    		int SoundTrack = dungeonOptionYaml.getInt("BGM.Normal");
