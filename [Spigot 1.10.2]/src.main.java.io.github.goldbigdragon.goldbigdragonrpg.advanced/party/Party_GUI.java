	...
	public void PartyGUI_Main(Player player)
	{
		...
		else
		{
			...
			if(main.Main_ServerOption.party.get(main.Main_ServerOption.partyJoiner.get(player)).getLeader().equalsIgnoreCase(player.getName()) == true)
			{
				...
				if(main.Main_ServerOption.party.get(main.Main_ServerOption.partyJoiner.get(player)).getLock() == false) Stack2("§9§l파티 개방", 54,0,1,Arrays.asList("§7파티 가입 신청을 받습니다."), 34, inv);
		...
		else
		{
			...
			for(int count = page*45; count < main.Main_ServerOption.party.size();count++)
			{
				...
				if(main.Main_ServerOption.party.get(a[count]).getLock()==false)
					Stack2("§f§l" + main.Main_ServerOption.party.get(a[count]).getTitle(), 54,0,1,Arrays.asList("§7파티 생성 시각 : "+ChatColor.DARK_GRAY+a[count],"","§b파티장 : §8"+main.Main_ServerOption.party.get(a[count]).getLeader(),"§b인원 : §8"+ main.Main_ServerOption.party.get(a[count]).getPartyMembers()+"/"+main.Main_ServerOption.party.get(a[count]).getCapacity()), loc, inv);
				else
					Stack2("§f§l" + main.Main_ServerOption.party.get(a[count]).getTitle(), 130,0,1,Arrays.asList("§7파티 생성 시각 : "+ChatColor.DARK_GRAY+a[count],"","§b파티장 : §8"+main.Main_ServerOption.party.get(a[count]).getLeader(),"§b인원 : §8"+ main.Main_ServerOption.party.get(a[count]).getPartyMembers()+"/"+main.Main_ServerOption.party.get(a[count]).getCapacity(),"","§c[잠김]","§c파티 가입을 하시려면","§c파티장에게 문의하세요!"), loc, inv);
				loc++;
			}
		}
