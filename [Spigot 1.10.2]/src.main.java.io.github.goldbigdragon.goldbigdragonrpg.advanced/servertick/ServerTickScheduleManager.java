...
public class ServerTickScheduleManager
{
	public void saveCategoriFile()
	{
		...
		for(int count = 0; count < ServerTickMain.DungeonSchedule.size(); count++)
		{
			...
			for(int counter=0; counter<ServerTickMain.DungeonSchedule.get(UTC).getDungeonMakerSize();counter++)
				PHSF.set("DungeonSchedule."+count+".DungeonMaker."+counter, ServerTickMain.DungeonSchedule.get(UTC).getDungeonMaker(counter));
			for(int counter=0; counter<ServerTickMain.DungeonSchedule.get(UTC).getGridSize();counter++)
			{
			...
			for(int counter=0; counter<ServerTickMain.DungeonSchedule.get(UTC).getKeyGridSize();counter++)
			{
	...
	public void loadCategoriFile()
	{
		...
		if(PHSF.contains("Schedule"))
		{
			for(int count = 0; count < PHSF.getConfigurationSection("Schedule").getKeys(false).size(); count++)
			{
				...
				ServerTickObject STSO = new ServerTickObject(UTC, PHSF.getString("Schedule."+count+".Type"));
