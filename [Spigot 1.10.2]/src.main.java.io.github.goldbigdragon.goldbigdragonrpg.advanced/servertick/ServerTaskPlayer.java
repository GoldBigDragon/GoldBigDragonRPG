...
public class ServerTaskPlayer
{
	public void UseTeleportScroll(long UTC)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					new effect.SendPacket().sendActionBar(user, "§e§l[텔레포트까지 남은 시간 : §f§l" + ServerTickMain.Schedule.get(UTC).getCount() +"§e§l초]");
	...
	public void ExChangeTimer(long UTC)
	{
		...
		if(ServerTickMain.Schedule.get(UTC).getCount() <= ServerTickMain.Schedule.get(UTC).getMaxCount())
		{
			...
			PS.sendTitleSubTitle(caller, "\'§e[교환 신청]\'", "\'"+TimerBar(ServerTickMain.Schedule.get(UTC).getCount(), 10)+"\'", (byte)1, (byte)0, (byte)1);
			PS.sendTitleSubTitle(target, "\'§e[교환 신청]\'", "\'"+TimerBar(ServerTickMain.Schedule.get(UTC).getCount(), 10)+"\'", (byte)1, (byte)0, (byte)1);
