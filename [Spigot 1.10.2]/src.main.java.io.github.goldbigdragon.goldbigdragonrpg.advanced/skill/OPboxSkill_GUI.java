...
public class OPboxSkill_GUI extends Util_GUI
{
	public void AllSkillsGUI(Player player, short page, boolean isJobGUI,String WhatJob)
	{
		...
		if(isJobGUI == false)
			Stack2("§f§l새 스킬", 386,0,1,Arrays.asList(ChatColor.GRAY + "새로운 스킬을 생성합니다."), 49, inv);
		Stack2("§f§l이전 목록", 323,0,1,Arrays.asList(ChatColor.GRAY + "이전 화면으로 돌아갑니다.",ChatColor.BLACK+WhatJob), 45, inv);
		Stack2("§f§l닫기", 324,0,1,Arrays.asList(ChatColor.GRAY + "창을 닫습니다.",ChatColor.BLACK+""+isJobGUI), 53, inv);
	...
	public void IndividualSkillOptionGUI(Player player, short page, String SkillName)
	{
		...
		if(page!=0)
			Stack2("§f§l이전 페이지", 323,0,1,Arrays.asList(ChatColor.GRAY + "이전 페이지로 이동 합니다."), 48, inv);
		Stack2("§f§l새 랭크", 386,0,1,Arrays.asList(ChatColor.GRAY + "새로운 스킬 랭크를 생성 합니다."), 49, inv);
		Stack2("§f§l이전 목록", 323,0,1,Arrays.asList(ChatColor.GRAY + "이전 화면으로 돌아갑니다."), 45, inv);
		Stack2("§f§l닫기", 324,0,1,Arrays.asList(ChatColor.GRAY + "창을 닫습니다.",ChatColor.BLACK+SkillName), 53, inv);
		player.openInventory(inv);
	}
	...
	public void SkillRankOptionGUI(Player player, String SkillName, short SkillLevel)
	{
		...
		Stack2("§f§l이전 목록", 323,0,1,Arrays.asList(ChatColor.GRAY + "이전 화면으로 돌아갑니다.",ChatColor.BLACK+""+SkillLevel), 45, inv);
		Stack2("§f§l닫기", 324,0,1,Arrays.asList(ChatColor.GRAY + "창을 닫습니다.",ChatColor.BLACK+SkillName), 53, inv);
		player.openInventory(inv);
	}
