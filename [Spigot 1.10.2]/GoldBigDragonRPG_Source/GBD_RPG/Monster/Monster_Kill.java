    				...
    				|| event.getEntity().getLastDamageCause().getCause() == DamageCause.MAGIC)
    		{
				if(Bukkit.getServer().getPlayer(event.getEntity().getKiller().getName()).isOnline() == true)
				{
					...
					if(GBD_RPG.Main_Main.Main_ServerOption.PlayerList.get(player.getUniqueId().toString()).isAlert_MobHealth())
					    new Effect_Packet().sendTitleSubTitle(player, "\'" +ChatColor.BLACK+"¡á¡á¡á¡á¡á¡á¡á¡á¡á¡á"+ "\'", "\'" +ChatColor.DARK_RED+""+ChatColor.BOLD+"[DEAD]"+ "\'", (byte)0, (byte)0, (byte)1);
    				Reward(event,player);
    				Quest(event, player);
					return;
				}
	...
	public void Reward(EntityDeathEvent event, Player player)
	{
		...
		if(40 <= N.RandomNum(0, 100) * Main_ServerOption.Event_DropChance)
		{
			...
			if(lucky >= N.RandomNum(0, 100))
			{
				...
				if(luckysize <= 80){player.sendMessage(ChatColor.YELLOW +""+ChatColor.BOLD+ "[SYSTEM] : ·°Å° ÇÇ´Ï½Ã!");amount = 2;	sound.SP(player, org.bukkit.Sound.ENTITY_PLAYER_LEVELUP, 0.5F, 0.9F);}
				else if(luckysize <= 95){player.sendMessage(ChatColor.YELLOW +""+ChatColor.BOLD+ "[SYSTEM] : ºò ·°Å° ÇÇ´Ï½Ã!");amount = 5;	sound.SP(player, org.bukkit.Sound.ENTITY_PLAYER_LEVELUP, 0.7F, 1.0F);}
				else{player.sendMessage(ChatColor.YELLOW +""+ChatColor.BOLD+ "[SYSTEM] : ÈÞÁî ·°Å° ÇÇ´Ï½Ã!");amount = 20;	sound.SP(player, org.bukkit.Sound.ENTITY_PLAYER_LEVELUP, 1.0F, 1.1F);}
			}
		}
		else
			amount = 0;
