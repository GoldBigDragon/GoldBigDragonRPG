			...
			if(player.getInventory().getItemInMainHand().getItemMeta().hasLore()==true)
			{
				...
				if(itemID == 383 && event.getItem().getData().getData() == 0 && event.getAction() == Action.RIGHT_CLICK_BLOCK)
				{
	    			if(GoldBigDragon_RPG.Main.ServerOption.MonsterList.containsKey(ChatColor.stripColor(event.getItem().getItemMeta().getDisplayName())))
	    			{
	    				...
	    				{
	    					new GoldBigDragon_RPG.Effect.Sound().SP(player, org.bukkit.Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 2.0F, 1.7F);
	...
	private void OPwork_Area(PlayerInteractEvent event)
	{
		...
		if(event.getAction()==Action.LEFT_CLICK_BLOCK)
		{
			if(u.getString(player, (byte)3).compareTo("ANBI") == 0)
			{
				...
				new GoldBigDragon_RPG.Effect.Sound().SP(player, Sound.ENTITY_HORSE_SADDLE, 1.0F, 1.8F);
		...
		else if(event.getAction()==Action.RIGHT_CLICK_BLOCK)
		{
			if(u.getString(player, (byte)3).compareTo("MLS")==0)//MonsterLocationSetting
			{
				...
				AreaConfig.saveConfig();
				new GoldBigDragon_RPG.Effect.Sound().SP(player, Sound.ENTITY_ITEM_PICKUP, 1.0F, 1.8F);
		    	u.clearAll(player);
	...
	private void OPwork_Gamble(PlayerInteractEvent event)
	{
		...
		else if(event.getAction()==Action.RIGHT_CLICK_BLOCK)
		{
			if(u.getString(player, (byte)0).compareTo("NSM")==0)//NewSlotMachine
			{
				...
				{
					new GoldBigDragon_RPG.Effect.Sound().SP(player, Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 1.0F, 1.8F);
				...
				new GoldBigDragon_RPG.Effect.Sound().SP(player, Sound.ENTITY_IRONGOLEM_DEATH, 1.0F, 1.8F);
		    	u.clearAll(player);
